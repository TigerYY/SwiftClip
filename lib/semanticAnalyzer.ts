...
export class SemanticAnalyzer {
  ...
  private static isRedundantContent(text: string): boolean {
    ...
    // 短且无意义
    const isShortAndMeaningless =
      words.length <= 3 && words.every(word => CHINESE_STOP_WORDS.has(word) || word.length === 1)
  ...
  private static generateCompressionPlan(
    ...
    // 计算重要内容总时长
    const importantDuration = importantSegments.reduce((sum, seg) => sum + (seg.end - seg.start), 0)

    // 计算冗余内容总时长
    const redundantDuration = redundantSegments.reduce((sum, seg) => sum + (seg.end - seg.start), 0)
